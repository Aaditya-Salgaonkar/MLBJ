    ...
    <div className="min-h-screen bg-[#F5F5F5] font-sans text-slate-800">
      ...
      <nav className="bg-white py-4 px-6 md:px-12 flex items-center justify-between sticky top-0 z-50 shadow-sm">
        ...
        <div className="hidden md:flex items-center gap-8 font-medium text-slate-600">
          <Link href="/" className="hover:text-[#A03623] transition-colors">Home</Link>
          <Link href="#services" className="hover:text-[#A03623] transition-colors">Services</Link>
          <Link href="#about" className="hover:text-[#A03623] transition-colors">About</Link>
          <Link href="#contact" className="hover:text-[#A03623] transition-colors">Contact us</Link>
        </div>
      ...
      <header className="relative pt-12 pb-24 px-6 md:px-12 max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="relative mt-8 lg:mt-0">
            ...
            <div className="absolute bottom-10 -left-6 md:-left-8 bg-white p-4 rounded-xl shadow-xl max-w-[150px] flex flex-col items-center text-center border border-slate-100">
            ...
            <div className="absolute -bottom-6 right-6 md:-right-6 bg-white p-4 rounded-xl shadow-xl max-w-[150px] flex flex-col items-center text-center border border-slate-100">
              <div className="mb-2 text-slate-800">
                <Scale size={28} />
              </div>
            ...
            <div className="absolute top-8 -right-6 md:-right-8 bg-white p-4 rounded-xl shadow-xl max-w-[150px] flex flex-col items-center text-center border border-slate-100">
              <div className="mb-2 text-slate-800">
                <FileText size={28} />
              </div>
      ...
      {/* Services Section */}
      <section id="services" className="bg-[#F5F5F5] py-20 px-6 md:px-12">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            <ServiceCard 
              title="Case Outcome Predictions"
              description="Upload case facts and FIR details to receive immediate statistical predictions on case outcomes."
              imageUrl="/outcome.png"
              features={[
                "Outcome forecasting",
                "Confidence scoring",
                "Relevant IPC mapping"
              ]}
            />
            <ServiceCard 
              title="Case Summarisation"
              description="Automatically parse complex legal documents and FIRs into clear, actionable summaries."
              imageUrl="/summary.jpg"
              features={[
                "Instant FIR parsing",
                "AI Section Tagging",
            ...
            />
            <ServiceCard 
              title="Similar Cases Retrieval"
              description="Instantly retrieve precedent judgments with high similarity to strengthen your legal arguments."
              imageUrl="/search.png"
              features={[
                "Semantic search",
      ...
      <footer className="bg-slate-900 text-slate-400 py-8 text-center text-sm">
        <p>&copy; {new Date().getFullYear()} ML Based Judiciary System. All rights reserved.</p>
      </footer>
    </div>
  );
}

// Typed Sub-component for Service Cards
...
function ServiceCard({ title, description, features, imageUrl }: ServiceCardProps) {
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col border border-slate-100">
      <div className="h-48 w-full relative group bg-slate-200">
         <img 
            src={imageUrl} 
            alt={title} 
            className="w-full h-full object-cover"
        ...
        <ul className="space-y-2 mb-8 flex-grow">
          {features.map((feature, idx) => (
            <li key={idx} className="flex items-start gap-2 text-xs text-slate-500 font-medium">
              <span className="mt-1 w-1.5 h-1.5 bg-[#A03623] rounded-full flex-shrink-0"></span>
              {feature}
            </li>
          ))}
        </ul>
